  ...
  "the toString function" when
  {
    ...
    "given a deck entry with card information" should
    {
      "return the correct string" in
      {
        ...
        val expected = """[1] Llanowar Scout {manacost: "{1}{G}", colors: "G", types: [Creature], cmc: 2.00} @cmc_2 @extra_land_drop @ramp @creature @green"""
    ...
    "given a deck entry without tags" should
    {
      "return the correct string" in
      {
        ...
        val expected = """[1] Llanowar Scout {manacost: "{1}{G}", colors: "G", types: [Creature], cmc: 2.00} """
        DeckEntryAdapter.toString(input) shouldEqual expected
      }
    }
  }
