...
object TypeAdapter
{
  ...
  private val ORDERED_TYPES =
      List(ENCHANTMENT, ARTIFACT, CREATURE, INSTANT, SORCERY, PLANESWALKER, LAND)
  ...
  def parseToSequence(str: String): Try[Seq[Type]] =
  {
    if (!str.matches(TYPE_SEQUENCE_REGEX.toString))
      ...
      Try(str.substring(1, str.length - 1).split(',').map(s => parse(s.trim)))
  }

  def parse(str: String): Type =
  {
    ORDERED_TYPES.find(regex => str.matches(regex.toString)) match
    {
